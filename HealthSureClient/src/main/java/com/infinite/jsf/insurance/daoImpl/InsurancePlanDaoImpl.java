...
public class InsurancePlanDaoImpl implements InsurancePlanDao {
    ...
    @Override
    public List<InsurancePlan> searchByRecipientIdAndDob(String recipientId, String dob) {
        List<InsurancePlan> eligiblePlans = new ArrayList<>();

        try {
            // Convert String DOB to LocalDate
            LocalDate dobDate = Date.valueOf(dob).toLocalDate();
            int age = Period.between(dobDate, LocalDate.now()).getYears();

            // Fetch all plans
            List<InsurancePlan> allPlans = showAllPlans();

            for (InsurancePlan plan : allPlans) {
                if (plan.getMinEntryAge() <= age && age <= plan.getMaxEntryAge()) {
                    eligiblePlans.add(plan);
                }
            }
        } catch (Exception e) {
            System.out.println("Error parsing DOB or calculating age: " + e.getMessage());
        }
